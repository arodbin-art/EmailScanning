...
## Source credentials
From `secrets/rodleeallen_secrets.json`:
...
GMAIL_TOKEN_STORE_PATH=secrets/gmail_tokens.json
...
4) Confirm refresh token stored in `secrets/gmail_tokens.json`.
...
## Notes
- Tokens are encrypted at rest in `secrets/gmail_tokens.json`.
